...
class L5SwaggerServiceProvider extends ServiceProvider
{
    ...
    public function boot()
    {
        ...
        $configPath = $legacy? 'l5-swagger.paths.views' : 'l5-swagger.documentations.'. $default .'.paths.views';

        //Publish views
        $this->publishes([
            __DIR__.'/../resources/views' => config($configPath),
        ], 'views');
    ...
    public function register()
    {
        ...
        $this->app->bind(Generator::class, function ($app) {
            $legacy = config('l5-swagger.legacy');
            $documentation = $legacy? 'legacy' : config('l5-swagger.default');
